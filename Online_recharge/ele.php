...
<div class="container">
	...
	<div class="details">
		<h1>Viewing Electricity Details</h1>
		<?php 
			$sql="SELECT * FROM ele";
			$res=$con->query($sql);
			if($res->num_rows > 0)
			{
				echo "<table border='1px'>
					<tr>
						<th>SNO</th>
						<th>ELECTRI CUSTOMER ID</th>
						<th>ELECREI CIRCLE</th>   
						<th>AMOUNT</th>
					</tr>";	
				$i=0;
				while ($row=$res->fetch_assoc())
			    {
					...
					echo "<tr>";
					echo "<td>{$i}</td>";
					echo "<td>{$row['ELE_CUS_ID']}</td>";
					echo "<td>{$row['ELE_CIRCLE']}</td>";
					echo "<td>{$row['ELE_AMOUNT']}</td>";
					echo "</tr";
					echo "</table>";
				}
